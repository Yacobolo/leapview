...
- Remote acquisition and credentials remain outside the node DuckDB instance. Connector adapters write private immutable staging artifacts, enforce row, byte, and file limits, validate content digests and schema, and atomically finalize them before governed materialization.
- Managed data owns staging retention and cleanup. DuckDB reads only finalized artifacts beneath approved staging and DuckLake roots.
- The node DuckDB instance loads a fixed extension allowlist at startup, disables arbitrary installation and autoloading, does not persist credentials in DuckDB's secret manager, and locks its access configuration before serving work. Infrastructure credentials come from the configured external secret boundary.
