...
- Arrow is the canonical in-process representation for governed analytical result batches through result limiting and cache retention. Cache entries own immutable Arrow buffers behind leases; eviction and runtime retirement release only the cache reference, while active consumers remain pinned. JSON, dashboard, agent, CLI, and Data Explorer adapters convert at their domain boundary, and control-plane contracts never depend on Arrow.
