...
func TestRefreshVisibilityStreamsAndPersistsSemanticModelRuns(t *testing.T) {
	...
	requireAssetRefreshStatus(t, pipelineStream, refreshrun.RunStatusRunning)
	requireAssetRefreshStatus(t, pipelineStream, refreshrun.RunStatusSucceeded)

	repo := materializesqlite.NewSQLRunRepository(h.store.SQLDB())
	pipelineRuns, err := repo.ListTargetRuns(ctx, workspaceID, refreshrun.TargetRefreshPipeline, "sales.sales-refresh", refreshrun.RunPage{Limit: 10})
	if err != nil {
	...
	root := pipelineRuns[0]
	if root.Status != refreshrun.RunStatusSucceeded || root.TriggerType != refreshrun.TriggerManual || root.PrincipalID != "dev" {
		t.Fatalf("refresh pipeline run = %#v, want succeeded manual run attributed to dev", root)
	}

	tableRuns, err := repo.ListTargetRuns(ctx, workspaceID, refreshrun.TargetModelTable, "sales.orders", refreshrun.RunPage{Limit: 10})
	if err != nil {
