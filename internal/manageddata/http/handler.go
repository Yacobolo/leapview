...
func (h *Handler) GetManagedDataEnvironmentRevision(w stdhttp.ResponseWriter, r *stdhttp.Request, project, connection, environment string) {
	...
	response.RolloutId = stringPointer(pointer.RolloutID)
...
func (h *Handler) ListManagedDataRollouts(w stdhttp.ResponseWriter, r *stdhttp.Request, project, connection string, params apigenapi.GenListManagedDataRolloutsParams) {
	if h.options.Rollouts == nil {
		h.writeUnavailable(w, r)
		return
	}
	collection, ok := h.collection(w, r, project, connection)
	...
	environment := valueOrEmpty(params.Environment)
	status := RolloutStatus("")
	if params.Status != nil {
		status = RolloutStatus(*params.Status)
	}
	if environment != "" && !validScopeID(environment) || status != "" && !validRolloutStatus(status) {
		h.writeError(w, r, ErrInvalid)
		return
	}
	rows, err := h.options.Rollouts.List(r.Context(), RolloutListRequest{Project: project, Connection: connection, CollectionID: collection.ID, Environment: environment, Status: status})
	if err != nil {
		h.writeError(w, r, err)
		return
	}
	...
	items := make([]apigenapi.ManagedDataRolloutSummaryResponse, 0, len(rows))
	for _, row := range rows {
		if row.CollectionID != collection.ID {
			h.writeError(w, r, errors.New("rollout coordinator returned an unrelated collection"))
			return
		}
		if environment != "" && row.Environment != environment || status != "" && row.Status != status {
			continue
		}
		item, mapErr := rolloutSummary(row)
		if mapErr != nil {
			h.writeError(w, r, mapErr)
			return
		}
	...
	scope := "rollouts\x00" + collection.ID + "\x00" + environment + "\x00" + string(status)
	page, next, err := pageSlice(items, params.Limit, params.PageToken, scope)
	if err != nil {
		h.writeError(w, r, err)
		return
	}
	h.writeJSON(w, stdhttp.StatusOK, apigenapi.ManagedDataRolloutListResponse{Items: page, Page: apigenapi.PageInfo{NextCursor: next}})
}

func (h *Handler) CreateManagedDataRollout(w stdhttp.ResponseWriter, r *stdhttp.Request, project, connection string, headers apigenapi.GenCreateManagedDataRolloutHeaders) {
	if h.options.Rollouts == nil {
		h.writeUnavailable(w, r)
		return
	}
	...
	if !validIdempotencyKey(headers.IdempotencyKey) {
		h.writeError(w, r, ErrInvalid)
		return
	}
	var body apigenapi.ManagedDataRolloutCreateRequest
	if err := h.decodeRequiredJSON(w, r, &body); err != nil {
		h.writeError(w, r, err)
		return
	}
	if !revisionPattern.MatchString(body.RevisionId) || !validScopeID(body.Environment) || len(body.Targets) < 1 {
		h.writeError(w, r, ErrInvalid)
		return
	}
	if len(body.Targets) > maxRolloutTargets {
		h.writeError(w, r, ErrTooLarge)
		return
	}
	...
	if revision.Revision.CollectionID != collection.ID || revision.Revision.Status != manageddata.RevisionStatusReady {
		h.writeError(w, r, ErrNotFound)
		return
	}
	targets := make([]RolloutTargetRequest, len(body.Targets))
	seen := make(map[string]struct{}, len(body.Targets))
	for i, target := range body.Targets {
		if !validScopeID(target.Workspace) || !validResourceID(target.ServingStateId, 160) {
			h.writeError(w, r, ErrInvalid)
			return
		}
		if _, exists := seen[target.Workspace]; exists {
			h.writeError(w, r, ErrInvalid)
			return
		}
		seen[target.Workspace] = struct{}{}
		targets[i] = RolloutTargetRequest{Workspace: target.Workspace, ServingStateID: target.ServingStateId}
	}
	actor, ok := h.actor(w, r)
	if !ok {
		return
	}
	result, err := h.options.Rollouts.Create(r.Context(), RolloutCreateRequest{
		Project: project, Connection: connection, CollectionID: collection.ID, RevisionID: body.RevisionId,
		Environment: body.Environment, Targets: targets, Actor: actor, IdempotencyKey: headers.IdempotencyKey,
	})
	if err != nil {
		h.writeError(w, r, err)
		return
	}
	if result.RevisionID != body.RevisionId || result.Environment != body.Environment {
		h.writeError(w, r, ErrNotFound)
		return
	}
	response, err := rolloutResponse(result, collection.ID, "")
	if err != nil {
		h.writeError(w, r, err)
		return
	}
...
func (h *Handler) GetManagedDataRollout(w stdhttp.ResponseWriter, r *stdhttp.Request, project, connection, rolloutID string) {
	h.rolloutMutation(w, r, project, connection, rolloutID, "", stdhttp.StatusOK, func(ctxRequest RolloutRequest) (Rollout, error) {
		return h.options.Rollouts.Get(r.Context(), ctxRequest)
	})
}

func (h *Handler) ActivateManagedDataRollout(w stdhttp.ResponseWriter, r *stdhttp.Request, project, connection, rolloutID string, headers apigenapi.GenActivateManagedDataRolloutHeaders) {
	h.rolloutMutation(w, r, project, connection, rolloutID, headers.IdempotencyKey, stdhttp.StatusAccepted, func(request RolloutRequest) (Rollout, error) {
		return h.options.Rollouts.Activate(r.Context(), request)
	})
}

func (h *Handler) RollbackManagedDataRollout(w stdhttp.ResponseWriter, r *stdhttp.Request, project, connection, rolloutID string, headers apigenapi.GenRollbackManagedDataRolloutHeaders) {
	if h.options.Rollouts == nil {
		h.writeUnavailable(w, r)
		return
	}
	var body apigenapi.ManagedDataRolloutRollbackRequest
	if err := h.decodeOptionalJSON(w, r, &body); err != nil {
		h.writeError(w, r, err)
		return
	}
	reason := valueOrEmpty(body.Reason)
	if len(reason) > 2048 || body.Reason != nil && strings.TrimSpace(reason) == "" {
		h.writeError(w, r, ErrInvalid)
		return
	}
	collection, request, ok := h.rolloutRequest(w, r, project, connection, rolloutID, headers.IdempotencyKey, true)
	if !ok {
		return
	}
	result, err := h.options.Rollouts.Rollback(r.Context(), RolloutRollbackRequest{RolloutRequest: request, Reason: reason})
	if err != nil {
		h.writeError(w, r, err)
		return
	}
	response, err := rolloutResponse(result, collection.ID, rolloutID)
	if err != nil {
		h.writeError(w, r, err)
		return
	}
...
func (h *Handler) rolloutMutation(w stdhttp.ResponseWriter, r *stdhttp.Request, project, connection, rolloutID, idempotencyKey string, status int, invoke func(RolloutRequest) (Rollout, error)) {
	if h.options.Rollouts == nil {
		h.writeUnavailable(w, r)
		return
	}
	mutation := status != stdhttp.StatusOK
	collection, request, ok := h.rolloutRequest(w, r, project, connection, rolloutID, idempotencyKey, mutation)
	if !ok {
		return
	}
...
func (h *Handler) rolloutRequest(w stdhttp.ResponseWriter, r *stdhttp.Request, project, connection, rolloutID, idempotencyKey string, mutation bool) (manageddata.Collection, RolloutRequest, bool) {
	collection, ok := h.collection(w, r, project, connection)
	if !ok {
		return manageddata.Collection{}, RolloutRequest{}, false
	}
	if !validResourceID(rolloutID, 160) || mutation && !validIdempotencyKey(idempotencyKey) {
		h.writeError(w, r, ErrInvalid)
		return manageddata.Collection{}, RolloutRequest{}, false
	}
	actor := ""
	...
	return collection, RolloutRequest{Project: project, Connection: connection, CollectionID: collection.ID, RolloutID: rolloutID, Actor: actor, IdempotencyKey: idempotencyKey}, true
}
...
func rolloutSummary(value Rollout) (apigenapi.ManagedDataRolloutSummaryResponse, error) {
	if !validRollout(value) || len(value.Targets) < 1 || len(value.Targets) > maxRolloutTargets {
		return apigenapi.ManagedDataRolloutSummaryResponse{}, errors.New("invalid rollout metadata")
	}
	return apigenapi.ManagedDataRolloutSummaryResponse{Id: value.ID, RevisionId: value.RevisionID, Environment: value.Environment, Status: apigenapi.ManagedDataRolloutStatus(value.Status), TargetCount: int32(len(value.Targets)), CreatedAt: value.CreatedAt, ActivatedAt: stringPointer(value.ActivatedAt), RolledBackAt: stringPointer(value.RolledBackAt), Error: safeRolloutError(value.Status)}, nil
}

func rolloutResponse(value Rollout, collectionID, expectedID string) (apigenapi.ManagedDataRolloutResponse, error) {
	if value.CollectionID != collectionID || expectedID != "" && value.ID != expectedID {
		return apigenapi.ManagedDataRolloutResponse{}, ErrNotFound
	}
	if !validRollout(value) || len(value.Targets) < 1 || len(value.Targets) > maxRolloutTargets {
		return apigenapi.ManagedDataRolloutResponse{}, errors.New("invalid rollout metadata")
	}
	targets := make([]apigenapi.ManagedDataRolloutTargetResponse, len(value.Targets))
	for i, target := range value.Targets {
		if !validScopeID(target.Workspace) || !validResourceID(target.ServingStateID, 160) || !validRolloutTargetStatus(target.Status) {
			return apigenapi.ManagedDataRolloutResponse{}, errors.New("invalid rollout target metadata")
		}
		targets[i] = apigenapi.ManagedDataRolloutTargetResponse{Workspace: target.Workspace, ServingStateId: target.ServingStateID, Status: apigenapi.ManagedDataRolloutTargetStatus(target.Status), PreviousRevisionId: stringPointer(target.PreviousRevisionID), ActivatedAt: stringPointer(target.ActivatedAt), RolledBackAt: stringPointer(target.RolledBackAt)}
		if target.Status == RolloutTargetStatusFailed {
			targets[i].Error = stringPointer("managed-data rollout target failed")
		}
	}
	return apigenapi.ManagedDataRolloutResponse{Id: value.ID, RevisionId: value.RevisionID, Environment: value.Environment, Status: apigenapi.ManagedDataRolloutStatus(value.Status), Targets: targets, CreatedAt: value.CreatedAt, ActivatedAt: stringPointer(value.ActivatedAt), RolledBackAt: stringPointer(value.RolledBackAt), Error: safeRolloutError(value.Status)}, nil
}

func safeRolloutError(status RolloutStatus) *string {
	if status == RolloutStatusFailed {
		return stringPointer("managed-data rollout failed")
	}
	return nil
}

func validRollout(value Rollout) bool {
	return validResourceID(value.ID, 160) && revisionPattern.MatchString(value.RevisionID) && validScopeID(value.Environment) && validRolloutStatus(value.Status) && value.CreatedAt != ""
}

func validRolloutStatus(status RolloutStatus) bool {
	switch status {
	case RolloutStatusDraft, RolloutStatusActivating, RolloutStatusActive, RolloutStatusFailed, RolloutStatusRollingBack, RolloutStatusRolledBack:
		return true
	default:
		return false
	}
...
func validRolloutTargetStatus(status RolloutTargetStatus) bool {
	switch status {
	case RolloutTargetStatusPending, RolloutTargetStatusActivating, RolloutTargetStatusActive, RolloutTargetStatusFailed, RolloutTargetStatusRollingBack, RolloutTargetStatusRolledBack:
		return true
	default:
		return false
	}
