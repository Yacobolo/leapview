...
func PlanProjectAgainstGraph(projectPath, workspaceID string, active workspace.AssetGraph) (ProjectPlan, error) {
	...
	for index := range plan.Workspaces {
		compiledWorkspace, ok := compiled.Workspace(plan.Workspaces[index].ID)
		if !ok {
			continue
		}
		changes, dependencyChanges, summary := diffAssetGraphs(compiledWorkspace.Metadata().Graph, active)
