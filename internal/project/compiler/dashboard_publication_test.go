...
func TestCompileProjectLoadsDashboardPublicationsAndDependencyClosure(t *testing.T) {
	...
	publications := mustCompiledWorkspace(t, compiled, "sales").Definition.Publications
...
`,
	...
	closure := mustCompiledWorkspace(t, compiled, "sales").Definition.Publications["website"].DependencyAssetIDs
	for _, want := range []string{
...
`,
	...
	subject := mustCompiledWorkspace(t, compiled, "sales").Definition.Access.DataPolicies["public-filter"].Subject
