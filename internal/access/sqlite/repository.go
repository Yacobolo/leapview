...
`, email, email, query, query, query)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&principal.ID, &principal.Email, &principal.DisplayName, &principal.CreatedAt, &principal.UpdatedAt); err != nil {
			return nil, err
		}
...
`)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&binding.ID, &binding.WorkspaceID, &subjectType, &binding.SubjectID, &principalID, &groupID, &email, &displayName, &groupName, &binding.Role, &binding.CreatedAt); err != nil {
			return nil, err
		}
		binding.SubjectType = access.SubjectType(subjectType)
		binding.PrincipalID = nullString(principalID)
		binding.GroupID = nullString(groupID)
		binding.Email = nullString(email)
		binding.DisplayName = nullString(displayName)
		binding.GroupName = nullString(groupName)
