...
func TestRepositoryReconcileAndClaimDueCoalescesCatchUp(t *testing.T) {
	...
	if err := repo.Reconcile(t.Context(), refreshschedule.ReconcileInput{
		...
		Pipelines: []refreshschedule.Definition{{ID: "sales-refresh", Name: "sales-refresh", SemanticModel: "sales", Schedules: []refreshschedule.Schedule{schedule}}},
		Now:       deployedAt,
	}); err != nil {
...
func TestRepositoryClaimDueDoesNotAdvanceAnotherEnvironment(t *testing.T) {
	...
	for _, environment := range []string{"dev", "prod"} {
		if err := repo.Reconcile(t.Context(), refreshschedule.ReconcileInput{
			...
			Pipelines: []refreshschedule.Definition{{ID: "sales-refresh", SemanticModel: "sales", Schedules: []refreshschedule.Schedule{schedule}}},
		}); err != nil {
			t.Fatal(err)
		}
	}
...
func TestRepositoryCoalescesSimultaneouslyDueScheduleEntries(t *testing.T) {
	...
	if err := repo.Reconcile(t.Context(), refreshschedule.ReconcileInput{
		...
		Pipelines: []refreshschedule.Definition{{ID: "sales-refresh", SemanticModel: "sales", Schedules: []refreshschedule.Schedule{morning, later}}},
	}); err != nil {
		t.Fatal(err)
	}
...
func TestRepositoryReconcileRemovesSupersededSchedules(t *testing.T) {
	...
	input := refreshschedule.ReconcileInput{WorkspaceID: "sales", Environment: "prod", ArtifactDigest: "old", Pipelines: []refreshschedule.Definition{{ID: "sales-refresh", SemanticModel: "sales", Schedules: []refreshschedule.Schedule{schedule}}}, Now: now}
...
`); err != nil {
	...
	want := refreshschedule.DataVersion{
		...
		Source: refreshschedule.DataVersionSourceRefresh, PipelineID: "sales-refresh", RunID: "run_1",
	}
