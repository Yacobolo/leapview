...
Go uses gomponents to render page shells and update-stream initializers. Datastar transports server-owned state and commands, beginning with the first bootstrap signal patch. Lit custom elements render application chrome, workspace/catalog pages, dashboards, filters, charts, tables, administration, data, and chat/agent surfaces.
...
Read [System architecture and boundaries](/docs/architecture/system) for the normative dependency, deployment, storage, and scaling rules. The system page owns cross-capability rules; [Storage architecture](/docs/storage-architecture) owns the detailed DuckLake and recovery contract. Continue with [Semantic model architecture](/docs/architecture/semantic-models), [UI architecture](/docs/architecture/ui), [Agent architecture](/docs/architecture/agent), [Runtime architecture](/docs/architecture/runtime), [Remote CI with Autback](/docs/architecture/autback), [Datastar signal flow](/docs/architecture/datastar), [Filter and slicer target architecture](/docs/architecture/filters-slicers), [Visualization target architecture](/docs/architecture/visual-plugins), [Geographic rendering decision](/docs/architecture/geographic-rendering), and [Infisical Go SDK decision](/docs/architecture/infisical-go-sdk) for deeper boundaries.
