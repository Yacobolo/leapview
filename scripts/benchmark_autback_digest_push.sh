...
evidence_dir="${EVIDENCE_DIR:-${runner_temp}/autback-digest-push-benchmark}"
...
for workload in site production; do
  ...
  for run_index in $(seq 0 "${measured_runs}"); do
    ...
    image_tag="${repository}:autback-benchmark-${github_run_id}"
...
if [[ -n "${GITHUB_STEP_SUMMARY:-}" ]]; then
  {
    echo '## Autback warm digest-push benchmark'
    echo
