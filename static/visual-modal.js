...
// web/components/visual-modal.ts
var VisualModal = class extends i4 {
  constructor() {
    super(...arguments);
    this.mode = "";
    this.detail = null;
    this.notice = "";
    this.handleVisualAction = (event) => {
  ...
  connectedCallback() {
    ...
    window.addEventListener("ld-visual-action", this.handleVisualAction);
    window.addEventListener("keydown", this.handleKeydown);
  }
  disconnectedCallback() {
    window.removeEventListener("ld-visual-action", this.handleVisualAction);
    window.removeEventListener("keydown", this.handleKeydown);
    super.disconnectedCallback();
  }
  render() {
    return b2`
      ${this.detail && this.mode ? this.renderDialog(this.detail, this.mode) : A}
      ${this.notice ? b2`<div class="notice" role="status">${this.notice}</div>` : A}
    `;
  }
  renderDialog(detail, mode) {
    return b2`
  ...
  renderFocus(detail) {
    if (detail.visualType === "chart") {
      ...
      return b2`<ld-echart class="focus-chart" .chart=${chart}></ld-echart>`;
    }
    return b2`<div class="focus-table">${this.renderData(detail)}</div>`;
  }
  renderData(detail) {
    const columns = detail.columns ?? [];
    const rows = detail.rows ?? [];
    if (columns.length === 0 || rows.length === 0) return b2`<div class="empty">No visual data</div>`;
    return b2`
      <div class="data-shell">
        ...
        <div class="data-scroll">
          <table>
            <thead>
              <tr>
                ${columns.map((column) => b2`<th class=${column.align === "right" ? "right" : ""}>${column.label}</th>`)}
              </tr>
            </thead>
            <tbody>
              ${rows.map((row) => b2`
                <tr>
                  ${columns.map((column) => b2`<td class=${column.align === "right" ? "right" : ""} title=${stringValue(row[column.key])}>${stringValue(row[column.key])}</td>`)}
                </tr>
              `)}
            </tbody>
          </table>
...
__decorateClass([
  r5()
], VisualModal.prototype, "mode", 2);
__decorateClass([
  r5()
], VisualModal.prototype, "detail", 2);
__decorateClass([
  r5()
], VisualModal.prototype, "notice", 2);
function chartPayloadFromRows(detail) {
  return {
    id: detail.visualId,
    title: detail.title,
    type: "bar",
...
function toDelimited(detail, delimiter) {
  const columns = detail.columns ?? [];
  const rows = detail.rows ?? [];
  return [
