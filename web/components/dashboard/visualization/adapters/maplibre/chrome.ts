...
.maplibregl-map{position:relative;overflow:hidden;font:12px/20px var(--ld-font-family-ui,system-ui);-webkit-tap-highlight-color:transparent}
...
.maplibregl-ctrl-group{overflow:hidden;border:1px solid var(--ld-line-default,#d0d7de);border-radius:6px;background:var(--ld-bg-panel,#fff);box-shadow:0 1px 3px rgba(31,35,40,.16)}
.maplibregl-ctrl-group button{position:relative;display:block;width:30px;height:30px;margin:0;padding:0;border:0;border-bottom:1px solid var(--ld-line-subtle,#d8dee4);outline:none;background:transparent;color:var(--ld-fg-default,#1f2328);font:600 20px/30px var(--ld-font-family-ui,system-ui);cursor:pointer}
.maplibregl-ctrl-group button:last-child{border-bottom:0}
.maplibregl-ctrl-group button:hover{background:color-mix(in srgb,var(--ld-bg-panel,#fff) 88%,var(--ld-fg-default,#1f2328) 12%)}
.maplibregl-ctrl-group button:focus-visible{z-index:1;box-shadow:inset 0 0 0 2px var(--ld-accent-fg,#0969da)}
